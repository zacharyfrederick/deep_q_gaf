...
from keras.layers import Activation, Conv3D, Flatten, Dropout, Reshape, Permute, Lambda
...
def build_deepmind_model(x):
    x = Conv3D(32, (8, 8), strides=(4, 4), activation='relu', data_format='channels_first')(x)
    x = Conv3D(64, (4, 4), strides=(1, 1), activation='relu', data_format='channels_first')(x)
    x = Conv3D(64, (3, 3), strides=(1, 1), activation='relu', data_format='channels_first')(x)
    return x
...
def build_paper_model(num_gpus=None):
    model = Sequential()
    model.add(
        Conv3D(32, (8, 8), strides=(4, 4), input_shape=(4, 30, 180), activation='relu', data_format='channels_first'))
    model.add(Activation('relu'))
    model.add(Con3D(64, (4, 4), strides=(1, 1), data_format='channels_first'))
    model.add(Activation('relu'))
    model.add(Conv3D(64, (3, 3), strides=(1, 1), data_format='channels_first'))
    model.add(Activation('relu'))
    model.add(Flatten())
